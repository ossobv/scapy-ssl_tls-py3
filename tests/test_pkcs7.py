...
class TestPKCS7Encoder(unittest.TestCase):
    ...
    def test_pkcs7_encoder_returns_expected_padding_on_short_block(self):
        ...
        self.assertEqual(pkcs7_data[len(data):], bytes([self.pkcs7.k - len(data)]) * (self.pkcs7.k - len(data)))
    ...
    def test_pkcs7_padding_only_is_returned_on_get_padding_call(self):
        ...
        self.assertEqual(bytes([len(pkcs7_padding)]) * len(pkcs7_padding), pkcs7_padding)
        self.assertEqual(b"%s%s" % (data, pkcs7_padding), self.pkcs7.encode(data))
